...
function getPresets(t: (k: string) => string) {
  return [
    ...
    { name: t('models.presetZhipu'), base_url: 'https://open.bigmodel.cn/api/paas/v4', model: 'glm-4-air' },
    { name: t('models.presetOllama'), base_url: 'http://localhost:11434/v1', model: 'llama3.1' },
  ];
}
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <PageHeader
        ...
        actions={<button className="btn" onClick={() => setEditing({ name: '', base_url: '', api_key: '', model: '', temperature: 0.3 })}><Plus size={16} />{t('models.addModel')}</button>}
      />
        ...
        {models.map((m, i) => (
          <GlowCard key={m.id} delay={i * 0.04}>
            ...
            <div className="flex items-center justify-between">
              <button className="btn-ghost" onClick={() => test(m.id)}><Zap size={14} />{t('models.testConn')}</button>
              {tests[m.id] && (
                <div className={`text-xs ${tests[m.id].loading ? 'text-slate-400' : tests[m.id].ok ? 'text-neon' : 'text-magenta'} max-w-[60%] text-right`}>
                  {tests[m.id].loading ? t('models.testing') : tests[m.id].ok ? '✓ ' + (tests[m.id].content || t('models.testOk')) : (
                    <span title={(tests[m.id].error || '') + (tests[m.id].cause ? '\n' + tests[m.id].cause : '')}>
                      ✗ {tests[m.id].error || t('models.testFail')}
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 overflow-y-auto">
          ...
          <div className="glass p-6 max-w-lg w-full">
            ...
            {!editing.id && (
              <div className="mb-4">
                <div className="label">{t('models.presetLabel')}</div>
